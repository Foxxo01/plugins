(function setupVersionBadge() {
    ...
    if (!selectedBadge) {
        console.error("[VersionBadge] Geçersiz rozet türü.");
        return;
    }
    ...
    if (!store?.getUserProfile) {
        console.error("[VersionBadge] UserProfileStore bulunamadı.");
        return;
    }

    const originalGetUserProfile = store.getUserProfile;
    ...
    console.log(`[VersionBadge] ${selectedBadge.description} rozeti aktif.`);
})();
